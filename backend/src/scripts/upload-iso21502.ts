...
async function uploadISO21502Sections() {
  try {
    console.log('🚀 Starting ISO 21502-2020 sections upload...');

    // Read the ISO 21502 sections JSON file
    const iso21502Path = path.join(process.cwd(), '..', 'ISO 21502-2020_ Project, programme and portfolio management - Guidance on project management.json');
    const sectionsData: ISO21502SectionData[] = JSON.parse(fs.readFileSync(iso21502Path, 'utf8'));
    
    console.log(`📊 Found ${sectionsData.length} ISO 21502-2020 sections to process`);

    // Create or update the ISO 21502 standard
    const iso21502Standard = await prisma.standard.upsert({
      where: { title: 'ISO 21502-2020 Project, programme and portfolio management - Guidance on project management' },
      update: {
        type: 'ISO',
        version: '2020',
        description: 'ISO 21502:2020 provides guidance on project management and is applicable to any type of project, regardless of complexity, size or duration'
      },
      create: {
    ...
    console.log(`✅ ISO 21502 Standard created/updated: ${iso21502Standard.title} (ID: ${iso21502Standard.id})`);
    ...
    for (const [chapterKey, chapterData] of sortedChapters) {
      const chapter = await prisma.chapter.upsert({
        where: {
          standardId_number: {
            standardId: iso21502Standard.id,
      ...
      try {
        ...
        await prisma.section.upsert({
          ...
          update: {
            ...
            chapterId: chapterId,
            anchorId: `iso21502-${sectionData.section_number.replace(/\./g, '-')}`
          },
          create: {
            standardId: iso21502Standard.id,
            chapterId: chapterId,
    ...
    const iso21502Chapters = await prisma.chapter.findMany({
      where: { standardId: iso21502Standard.id },
      orderBy: { number: 'asc' },
      include: {
        sections: {
    ...
    console.log(`\n📚 ${iso21502Standard.title}:`);
    for (const chapter of iso21502Chapters) {
...
if (require.main === module) {
  uploadISO21502Sections()
    .then(() => {
      console.log('✅ ISO 21502-2020 upload script completed successfully');
      process.exit(0);
    })
    .catch((error) => {
      console.error('❌ ISO 21502-2020 upload script failed:', error);
      process.exit(1);
    });
}

export { uploadISO21502Sections };